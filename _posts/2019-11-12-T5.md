...
1. 프리트레이닝 데이터 셋이 성능에 미치는 영향을 확인
    ...
    - **Architectures**
        ...
        - 일반 Original Encoder-Decoder가 가장 좋았음
        - Computational Cost 유사해서 크게 문제..
    ...
    - **Datasets**
        ...
        - C4 휴리스틱 클린 진행
        - Specific한 도메인에서는 당연히 특정 테스크에서는 좋지만 데이터가 적어, 문제
    - **Training strategies**
    ...
    - **Scaling**
        ...
        - Single model 보다는 파인튜닝 한거보다는 좋았다.
        - Pretraining된거 하나랑 4개 따로
        - Pretraining 각각 4개 한 방향
    - **Pushing the limits**
        - We combined our above insights and trained substantially larger models (**up to 11 billion parameters**) to achieve state-of-the-art results across many of the benchmarks we considered. For unsupervised training, we extracted text from our C4 dataset and applied a denoising objective that corrupts contiguous spans of tokens. We pre-trained on a multi-task mixture before fine-tuning on individual tasks.
        - Overall, our models were trained on over 1 trillion tokens. In the interest of facilitating the replication, extension, and application of our results, we release our code, the C4 dataset, and pre-trained model weights for each T5 variant.
    ...
    1. 그 외에도 WSC를 위해서는 Ture 라벨만 사용한다던지 등의, 테스크별 추가 변환을 진행한 것들이 있음
